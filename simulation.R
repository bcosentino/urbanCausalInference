...
out_long <- out_long |>
  ...
  mutate(
    ...
    facet_lab = factor(facet_lab, levels = unique(paste0("Model: ", heading_models, "\n", heading_accuracy))),
...
p <- ggplot(out_long, aes(x = beta_h, fill = validity)) +
  ...
  annotate("text", x = b.sh,     y = 300, label = "True direct", vjust = 1.3, size = 3.3) +
  annotate("text", x = b.tot.sh, y = 300, label = "True total",  vjust = 1.3, size = 3.3) +
  facet_wrap(~ facet_lab, ncol = 2, scales = "fixed") +
